...
final class PremiumSheetViewController<View: PremiumViewInput>:
    ...
    private func updatePreferredContentSize() {
        ...
        guard screenWidth.isFinite && !screenWidth.isNaN && screenWidth > 0 &&
                currentHeight.isFinite && !currentHeight.isNaN && currentHeight > 0 else {
            print("⚠️ Warning: Invalid dimensions in updatePreferredContentSize - width: \(screenWidth), height: \(currentHeight)")
            let safeSize = CGSize(width: 375, height: 600) // Безопасные размеры по умолчанию
            scrollView.contentSize = safeSize
            preferredContentSize = safeSize
            return
        }
        
        let contentSize = CGSize(width: screenWidth, height: currentHeight)
